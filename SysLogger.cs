    ...
    public class Socket
    {
        ...
        private async void SendSource(LogModel logModel, SendType sendType)
        {
            try
            {
                ...
                else if (SysLogFormat == LogFormat.CustomSys)
                {
                    message = "<" + priorityNumber + ">" + ServerItem.AppName + ": " + logModel.Title + "" + logModel.Description + " | " + logModel.JsonDetail;
                }
                else
                {
                ...
                Task.Factory.StartNew(() =>
                {
                    ...
                    if (lastConnectStatus) // Save on socket server
                    {
                        ...
                        SysLogSocket.Send(Encoding.ASCII.GetBytes(message.ClearTurkishCharacter() + "\r\n"), SocketFlags.None);
                            ...
                            foreach (string fileItem in Directory.GetFiles(ServerItem.TextLogPath))
                            {
                                ...
                                foreach (string logItem in allText)
                                {
                                    try
                                    {
                                        ...
                                        {
                                            SysLogSocket.Send(Encoding.ASCII.GetBytes(logItem.ClearTurkishCharacter() + "\r\n"), SocketFlags.None);
                                            if (sendType == SendType.SendAndClose)
                                            {
                                                Disconnect();
                                                lastConnectStatus = false;
